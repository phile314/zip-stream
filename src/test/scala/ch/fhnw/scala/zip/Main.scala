...
object Main extends App {
    val content: Process[Task, ZipEntry[Task]] = Process.empty // Process.emit(ZipEntry(ZipEntryHeader("Test.txt", DateTime.now(), None), ZipDataByteVector(ByteVector.encodeUtf8("sfasdf").right.get)))
